...
from sqlalchemy import (
    Boolean, CheckConstraint, Column, Date, Index, Integer, String,
    ForeignKey, Time, UniqueConstraint, DateTime, Text, BigInteger
)
...
class GroupCourse(Base):
    ...
    type = Column(String(20), nullable=False, default="lecture")
...
class Student(Base):
    ...
    # rfid_uid е вече незадължително — телефонът замества физическата карта
    rfid_uid    = Column(String(50), unique=True, nullable=True)
    name        = Column(String(100), nullable=False)
    group_id    = Column(Integer, ForeignKey("Groups.id"), nullable=False)

    # ──── NFC / HCE криптография ────
    # 32 случайни байта, съхранени като 64-символен hex низ.
    # Генерира се от сървъра при първото /auth/provision-key.
    hmac_key    = Column(String(64), nullable=True)
    # Монотонно нарастващ брояч — предотвратява replay атаки.
    # Използваме BigInteger за да не се препълни никога.
    atc_counter = Column(BigInteger, nullable=False, default=0)
...
class Schedule(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint("group_course_id", "day_of_week", "start_time",
                         "is_biweekly", "subgroup", name="uq_schedule_slot"),
    )
